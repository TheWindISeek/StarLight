 ...
 * @file breath.c/UTF-8
 * @author @TheWindISeek
 * @brief 实现LED的呼吸效果 从最暗到最亮 从最亮到最暗
 * @version 0.1
 * @date 2023-05-03
 * 
...
//呼吸灯
// void LED_breath(unsigned int x);
void LED_light(void);
void LED_dark(void);
...
{  
    ...
    while(1){
        ...
        //         break;
        //     case 2:
		// 		PWM_ON = (PWM_ON - PWM_LEVEL <= 0) ? PWM_ON : PWM_ON - PWM_LEVEL;
        //         break;
        //     case 3:
        //         break;
        //     case 4:
        ...
        pre_on = PWM_ON;
        for(on = PWM_ON; on < light_end; on += PWM_LEVEL) {
            PWM_ON = on;
            delay(800+on);
        }		 
		on = light_end - PWM_LEVEL;
		for(;on>=pre_on; on-=PWM_LEVEL) {
			PWM_ON = on;
			delay(800+on);
		}
        PWM_ON = pre_on;
